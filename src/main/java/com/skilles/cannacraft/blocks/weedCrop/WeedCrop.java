...
public class WeedCrop extends PlantBlock implements BlockEntityProvider, Fertilizable { // custom crop block implementation (very WIP)
    ...
    protected static CompoundTag trimTag(CompoundTag tag) {
        return trimTag(tag, 0);
    }

    private static CompoundTag trimTag(CompoundTag tag, int flag) {
        CompoundTag newTag = tag;
        if (tag != null) {
            newTag.remove("id");
            newTag.remove("x");
            newTag.remove("y");
    ...
    public BlockState withBreeding(BlockState state, boolean breeding) {
        return state.with(BREEDING, breeding);
    }

    @Override
    public BlockEntity createBlockEntity(BlockView world) {
        return new WeedCropEntity();
    }
        ...
        if (world.isAir(pos.up())) { // if block above is air
            if (getMaxAge(state) == 7) { // if block is 1st stage
                if (state.get((this.getAgeProperty())) >= getMaxAge(state)) { // fully grown, set above to stage 2
                    ...
                    world.getBlockEntity(pos.up()).fromTag(world.getBlockState(pos.up()), world.getBlockEntity(pos).toTag(new CompoundTag()));
                    world.markDirty(pos.up(), world.getBlockEntity(pos.up()));
                } else {
    ...
    /* List<ItemStack> implementation (for future drop modifiers)
        ...
        public void onBreak(World world, BlockPos pos, BlockState state, PlayerEntity player) {
            if(state.get(MATURE)) {
                ItemStack newStack = new ItemStack(ModItems.WEED_FRUIT);
                tag = world.getBlockEntity(pos).toTag(new CompoundTag());
    ...
    public void onBreak(World world, BlockPos pos, BlockState state, PlayerEntity player) { // drops cannabis with BE's NBT
        boolean brokenWithShears = player.getMainHandStack().isItemEqual(Items.SHEARS.getDefaultStack());
    ...
    private void breedCrops(World world, BlockPos pos, Random random) {
        CompoundTag ogTag = world.getBlockEntity(pos).toTag(new CompoundTag());
        if (!ogTag.getBoolean("Male")) {
            ...
            for (Direction direction : Direction.Type.HORIZONTAL) {
                ...
                if (blockEntity2 instanceof WeedCropEntity) {
                    CompoundTag tag = blockEntity2.toTag(new CompoundTag());
            ...
            if (COUNT > 0) {
                ...
                // Save nbt
                world.getBlockEntity(pos).fromTag(world.getBlockState(pos), ogTag);
                world.markDirty(pos, world.getBlockEntity(pos));

                ogTag = world.getBlockEntity(pos).toTag(new CompoundTag());
    ...
    public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
        ...
        if (isBreeding(state)) {
            ...
            world.addParticle(new DustParticleEffect(r, g, b, .7f), pos.getX() + 0.5 + i,
                ...
                if (world.isAir(pos.up())) { // if block above is air
                    if (getMaxAge(state) == 7) { // if block is 1st stage
                        ...
                        if (random.nextInt((int) (25.0F / f) + 1) == 0) {
                            ...
                            world.getBlockEntity(pos).fromTag(world.getBlockState(pos), tag);
                            world.getBlockEntity(pos.up()).fromTag(world.getBlockState(pos.up()), tag);
                            world.markDirty(pos.up(), world.getBlockEntity(pos.up()));
                            world.markDirty(pos, world.getBlockEntity(pos));
                        }
                    }
    ...
    public void onPlaced(World world, BlockPos pos, BlockState state, @Nullable LivingEntity placer, ItemStack itemStack) {
        if (itemStack.hasTag()) {
            CompoundTag tag = itemStack.getSubTag("cannacraft:strain");
