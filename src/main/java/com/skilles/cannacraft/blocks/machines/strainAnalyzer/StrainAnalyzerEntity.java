...
public class StrainAnalyzerEntity extends MachineBlockEntity implements Tickable {
    ...
    public StrainAnalyzerEntity() {
        super(ModEntities.STRAIN_ANALYZER_ENTITY, DefaultedList.ofSize(2, ItemStack.EMPTY));
    ...
    public static boolean canCraft(DefaultedList<ItemStack> inventory) {
        ...
        if (stack.isItemEqual(ModItems.WEED_SEED.getDefaultStack()) && stack.getCount() >= 1 && stack.hasTag() && !stack.getSubTag("cannacraft:strain").getBoolean("Identified")) {
            CompoundTag outputTag = output.copy().getSubTag("cannacraft:strain");
            CompoundTag subTag = stack.copy().getSubTag("cannacraft:strain");
    ...
    public static void craft(DefaultedList<ItemStack> inventory) {
        ...
        if (outputSlot.isEmpty()) {
            CompoundTag strainTag = tag.getCompound("cannacraft:strain").copy();
    ...
    public void tick() {
        ...
        StrainAnalyzerEntity blockEntity = this;
